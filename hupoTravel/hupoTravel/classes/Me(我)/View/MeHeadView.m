...
//        self.image = [UIImage imageNamed:@"bg1"];
        [self sd_setImageWithURL:[NSURL URLWithString:@"http://inspiration.chanyouji.cn/UserActivityContent/207228/73740da8a9081a6410fcf0a1537e7bc0.jpg"] placeholderImage:[UIImage imageNamed:@"anno_placeholder"] options:SDWebImageProgressiveDownload];
        //1.添加头像
...
//        fanLabel.backgroundColor = [UIColor magentaColor];
        ...
        rightButton.top = 16;
        rightButton.right = kScreenWidth - 8;
        [rightButton setTitle:@"账号登陆" forState:UIControlStateNormal];
        [rightButton addTarget:self action:@selector(loginAction:) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:rightButton];
        self.loginButton = rightButton;
    }
    return self;
}
...
{
    button.enabled = NO;
    [button setTitle:@"已登录" forState:UIControlStateNormal];
    //这里开始跳转到登陆界面
    //回到controller跳转
