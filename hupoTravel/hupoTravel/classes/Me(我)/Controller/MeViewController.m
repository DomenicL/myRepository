...
- (void)viewDidLoad {
    ...
    [HPNotificationCenter addObserver:self selector:@selector(loginOut:) name:@"loginOut" object:nil];
...
- (void)loginOut:(NSNotificationCenter *)noti
{
    [self.headView.loginButton setTitle:@"账号登陆" forState:UIControlStateNormal];
    self.headView.loginButton.enabled = YES;
}
...
- (void)requestHeadViewData
{
    ...
    if (!account) {
        ...
        //没有数据的话
        MeUserModel *model = [[MeUserModel alloc] init];
        /**
         * 用户昵称
         */
        model.screen_name = @"暂无名字";
        /**
         * 用户头像地址（大图），180×180像素
         */
        model.avatar_large = nil;
        /**
         * 性别，m：男、f：女、n：未知
         */
        model.gender = @"m";
    ...
    [self.headView.loginButton setTitle:@"已登录" forState:UIControlStateNormal];
...
- (void)loginAction
{
    [self.navigationController pushViewController:[[OAuthViewController alloc] init] animated:YES];
}
